...
The embedded Kotlin has been updated from 2.0.21 to link:https://github.com/JetBrains/kotlin/releases/tag/v2.1.20[Kotlin 2.1.20].

==== Scala plugins no longer create unresolvable configurations

The Scala plugins used configurations named `incrementalScalaAnalysisFor` to resolve incremental analysis information from other projects.
Due to the way the configurations were created, they were unresolvable and produced errors in the `dependencies` report.
